...
namespace VectorAndMatrixNameSpace
{
	...
	*/
	cGSLVector& cGSLVector::operator +(double theVal) 
	{
		...
		if (mvVect != NULL)
		{	
		cGSLVector myTempVect(*this) ;
			myTempVect += theVal ;
			ourTempVector = myTempVect ; 
	...
	*/
	...
	{
	cGSLVector myTempVect(theVect2) ;
		myTempVect += *theVect1 ;
		ourTempVector = myTempVect ; 
		return ourTempVector ;
	}
	...
	*/
	cGSLVector operator -(double* theVect1, const cGSLVector& theVect2)
	{
	cGSLVector myTempVect(theVect2.GetSize(), *theVect1) ;
		myTempVect -= theVect2 ;
		ourTempVector = myTempVect ; 
		return ourTempVector ;
	}
	...
	*/
	...
	{	
	uint mySize = theVect.GetSize() ;
	cGSLVector myTempVect(mySize, theVal) ;
		for (register uint i = 0 ; i < mySize ; i++)
			myTempVect[i] /= theVect[i] ;
		ourTempVector = myTempVect ; 
		return ourTempVector ; 
	}
